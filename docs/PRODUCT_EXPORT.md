...
Admin: System > Scheduler > Schedule Configuration look for 'eb2cproduct_image_export_feed', click check-box, select runNow and then click submit
...
| XML Nodes/Attributes                                 | Magento Product Image Data                                                                                                                           |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| /ItemImages[@imageDomain]                            | parse_url(Mage::app()->getStore($storeId)->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB))['host']                                                  |
| /ItemImages[@clientId]                               | eb2ccore/etc/config.xml (eb2ccore/general/client_id)                                                                                                 |
| /ItemImages[@timestamp]                              | current date (xsd:date Mage::getModel('core/date')->date('c'))                                                                                       |
| /ItemImages/MessageHeader                            | Mage::helper('eb2cproduct')->generateMessageHeader($cfg->imageFeedEventType)                                                                         |
| /ItemImages/Item[@id]                                | $product->getSku() (not currently being de-normalized???)                                                                                            |
| /ItemImages/Item[@id]/Images/Image[@imageview]       | magento image_media type (image, small_image, thumbnail)                                                                                             |
| /ItemImages/Item[@id]/Images/Image[@imagename]       | magento image label ($product->getMediaGalleryImages()[Varien_Data_Collection][Varien_Object]->getLabel())                                           |
| /ItemImages/Item[@id]/Images/Image[@imageurl]        | magento image url ($product->getMediaGalleryImages()[Varien_Data_Collection][Varien_Object]->getUrl())                                               |
| /ItemImages/Item[@id]/Images/Image[@imagewidth       | magento image getimagesize(path|url) (getimagesize($product->getMediaGalleryImages()[Varien_Data_Collection][Varien_Object]->getPath()|getUrl()))    |
| /ItemImages/Item[@id]/Images/Image[@imageheight]     | magento image getimagesize(path|url) (getimagesize($product->getMediaGalleryImages()[Varien_Data_Collection][Varien_Object]->getPath()|getUrl()))    |


FYI: if the products in Magento have no image data then the image export will not generate any feed files.
