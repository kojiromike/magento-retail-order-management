...
class EbayEnterprise_Address_Test_Model_Validation_ResponseTest
	extends EcomDev_PHPUnit_Test_Case
{
	...
	{
		$response = Mage::getModel('ebayenterprise_address/validation_response');
	...
	{
		$mockLog = $this->getHelperMock('ebayenterprise_magelog/data', array('logWarn'));
		$mockLog->expects($this->once())
			->method('logWarn')
			->with($this->identicalTo($logMessage), $this->contains('EbayEnterprise_Address_Model_Validation_Response'))
			->will($this->returnSelf());
		$this->replaceByMock('helper', 'ebayenterprise_magelog', $mockLog);
		$response = Mage::getModel('ebayenterprise_address/validation_response');
		$response->setMessage($message);
		$this->assertEquals((bool) $valid, $response->isAddressValid());
	}
