...
class EbayEnterprise_Amqp_Test_Helper_ConfigTest
	...
	public function testGetQueueConfigurationScopes()
	{
		...
		$app->expects($this->any())
			...
			->will($this->returnValue(array_map(
				...
				function ($storeAndConfig) { return $storeAndConfig['store']; },
				$stores
			)));
		...
		$coreHelper->expects($this->any())
			...
			->will($this->returnValueMap(array_map(
				...
				function ($storeAndConfig) { return array($storeAndConfig['store'], $storeAndConfig['core_config']); },
				$stores
			)));
		...
		$helper->expects($this->any())
			...
			->will($this->returnValueMap(array_map(
				...
				// configuration for a given store.
				function ($storeAndConfig) { return array($storeAndConfig['store'], $storeAndConfig['amqp_config']); },
				$stores
			)));
