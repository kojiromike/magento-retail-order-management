 ...
 *   archive folder.
 */
class EbayEnterprise_Catalog_Model_Error_Confirmations
{
	...
	protected function _getLangCode()
	{
		if (trim($this->_defaultLangCode) === '') {
			$this->_defaultLangCode = Mage::helper('ebayenterprise_catalog')->getDefaultLanguageCode();
		}
	...
	public function initFeed($feedType)
	{
		$this->append(self::XML_DIRECTIVE)
			...
			->append(Mage::helper('ebayenterprise_catalog')->generateMessageHeader($feedType));
	...
	{
		$this->_queueMessage[] = Mage::helper('ebayenterprise_catalog')->mapPattern(
			array(
	...
	public function addError($type, $fileName)
	{
		$this->_queueError[] = sprintf(
			"%s\n%s\n%s",
			Mage::helper('ebayenterprise_catalog')->mapPattern($this->_getErrorTemplate($type, $fileName), self::XML_ERROR_OPEN_NODE),
			implode("\n", $this->_queueMessage),
	...
	public function addErrorConfirmation($sku)
	{
		$this->_queueConfirmation[] = sprintf(
			"%s\n%s\n%s",
			Mage::helper('ebayenterprise_catalog')->mapPattern(array('sku' => $sku), self::XML_ERRCONFIRMATION_OPEN_NODE),
			implode("\n", $this->_queueError),
	...
	public function process(Varien_Event_Observer $observer)
	{
		...
		$coreFeed = Mage::getModel(
			'ebayenterprise_catalog/feed_core',
