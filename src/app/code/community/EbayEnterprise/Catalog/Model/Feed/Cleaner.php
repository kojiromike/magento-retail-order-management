 ...
 * product links.
 */
class EbayEnterprise_Catalog_Model_Feed_Cleaner
{
        ...
        if (isset($args['products'])) {
            if (!$args['products'] instanceof Mage_Catalog_Model_Resource_Product_Collection) {
                ...
                trigger_error(sprintf(
                    '"products" must be an instance of Mage_Catalog_Model_Resource_Product_Collection. %s given.',
                    gettype($args['products'])
                ), E_RECOVERABLE_ERROR);
            }
    ...
    protected function _getAllParentConfigurableSkus(Varien_Data_Collection $productCollection)
    {
        ...
        return array_map(
            function ($product) use ($helper, $catalogId) {
        ...
        if (!empty($missingLinks)) {
            ...
            foreach (array_keys($this->_linkTypes) as $linkType) {
                $unresolvedLinks = array_merge(
                    ...
                    $this->_linkProducts(
                        $product,
                        array_filter($missingLinks, function ($el) use ($linkType) {
                            return $linkType === $el['link_type'];
                        }),
                        $linkType
                    )
                );
    ...
    protected function _addUsedProducts(Mage_Catalog_Model_Product $product)
    {
        ...
        foreach ($this->_products as $collectionProduct) {
            ...
            if ($collectionProduct->getTypeId() === Mage_Catalog_Model_Product_Type::TYPE_SIMPLE &&
