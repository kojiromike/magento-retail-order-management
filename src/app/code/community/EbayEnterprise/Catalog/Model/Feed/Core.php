 ...
 */
class EbayEnterprise_Catalog_Model_Feed_Core extends Varien_Object
{
     ...
     * @return self
     * @throws EbayEnterprise_Catalog_Exception_Feed_File if config is invalid
     */
    protected function _validateFeedConfig()
    {
        $feedConfig = $this->getFeedConfig();
        if (!is_array($feedConfig)) {
            throw new EbayEnterprise_Catalog_Exception_Feed_File(
    ...
    protected function _setCheckAndCreateDir($path, $dirName = null)
    {
        try {
            ...
            throw new EbayEnterprise_Catalog_Exception_Feed_File($e->getMessage());
        }
        if ($dirName) {
     ...
     * @return self
     * @throws EbayEnterprise_Catalog_Exception_Feed_File if file could not be moved
     */
    protected function _mv($srcFile, $targetFile)
    {
        if (!$this->getFsTool()->mv($srcFile, $targetFile)) {
            throw new EbayEnterprise_Catalog_Exception_Feed_File("Could not move {$srcFile} to {$targetFile}.");
        }
        return $this;
    }
    /**
     ...
     * @param  string $srcFile
     * @return string
     * @throws EbayEnterprise_Catalog_Exception_Feed_File if no sent file has been configured
     */
    public function mvToSentDirectory($srcFile)
    {
        if (!$this->getSentDirectory()) {
            throw new EbayEnterprise_Catalog_Exception_Feed_File('No sent directory configured');
        }
