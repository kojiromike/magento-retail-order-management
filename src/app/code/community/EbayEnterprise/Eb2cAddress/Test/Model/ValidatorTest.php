...
class EbayEnterprise_Eb2cAddress_Test_Model_ValidatorTest
...
<AddressValidationResponse xmlns="http://api.gsicommerce.com/schema/checkout/1.0">
	...
	protected function _setupSessionWithSuggestions($original, $suggestions, $hasFreshSuggestions=true)
	{
		...
		$addresses = new EbayEnterprise_Eb2cAddress_Model_Suggestion_Group();
		...
		$this->_getSession()
			->setData(EbayEnterprise_Eb2cAddress_Model_Validator::SESSION_KEY, $addresses);
	}
	...
	{
		$expectedType = 'EbayEnterprise_Eb2cAddress_Model_Suggestion_Group';
		$session = $this->_getSession();
		$sessionKey = EbayEnterprise_Eb2cAddress_Model_Validator::SESSION_KEY;
		...
		$session->setData($sessionKey, new EbayEnterprise_Eb2cAddress_Model_Suggestion_Group());
		$this->assertInstanceOf($expectedType, $validator->getAddressCollection());
	...
	public function testUpdateAddressWithSelections($postValue)
	{
		...
		Mage::app()->getRequest()->setPost(EbayEnterprise_Eb2cAddress_Block_Suggestions::SUGGESTION_INPUT_NAME, $postValue);
	...
	public function testValidateAddressMultiSuggestions()
	{
		...
		$this->assertSame(
			EbayEnterprise_Eb2cAddress_Model_Validator::SUGGESTIONS_ERROR_MESSAGE,
			$errorMessage,
	...
	public function testWithValidatedAddress()
	{
		...
		$session = $this->_getSession();
		$session->setData(EbayEnterprise_Eb2cAddress_Model_Validator::SESSION_KEY, 'this should be cleared out');
		$validator = Mage::getModel('eb2caddress/validator');
		...
		$this->assertInstanceOf('EbayEnterprise_Eb2cAddress_Model_Suggestion_Group', $session->getData(EbayEnterprise_Eb2cAddress_Model_Validator::SESSION_KEY));
	}
	...
	public function testSessionInteractionsNoValidationNecessary($useForShipping)
	{
		...
		// properly set
		$session = $this->_getSession();
		$group = $session->getData(EbayEnterprise_Eb2cAddress_Model_Validator::SESSION_KEY);
	...
	public function errorMessageWithInvalidMessageAndNoSuggestions()
	{
		...
		$this->assertSame(
			$errorMessage,
			EbayEnterprise_Eb2cAddress_Model_Validator::NO_SUGGESTIONS_ERROR_MESSAGE
		);
	}
	...
	public function testGetOriginalAddress()
	{
		...
		// create the EbayEnterprise_Eb2cAddress_Model_Suggestion_Group that stores address data in the session
		$addresses = new EbayEnterprise_Eb2cAddress_Model_Suggestion_Group();
		$addresses->setOriginalAddress($origAddress);
		// add the address data to the session
		$this->_getSession()->setData(
			EbayEnterprise_Eb2cAddress_Model_Validator::SESSION_KEY,
			$addresses
		);
	...
	public function testGetSuggestedAddresses()
	{
		...
		// create the EbayEnterprise_Eb2cAddress_Model_Suggestion_Group that stores the address data in the session
		$addressCollection = new EbayEnterprise_Eb2cAddress_Model_Suggestion_Group();
