<?php
class EbayEnterprise_Eb2cCore_Test_Model_FeedTest extends EbayEnterprise_Eb2cCore_Test_Base
{
	/**
	...
	public function testConstructInvalidConfig()
	{
		...
		$feedModelMock->expects($this->once())
			->method('_validateFeedConfig')
			->will($this->throwException(new EbayEnterprise_Eb2cCore_Exception_Feed_File()));
	...
	public function provideFeedConfigForValidation()
	{
		return array(
			...
			array(array(), "EbayEnterprise_Eb2cCore_Model_Feed missing configuration: 'local_directory', 'event_type'."),
			array('this is not right', "EbayEnterprise_Eb2cCore_Model_Feed 'feed_config' must be an array of feed configuration values.")
		);
	}
	/**
	...
	public function testMvToSentNoSentConfigured()
	{
		...
		$feed->expects($this->never())
			->method('_mv');
		$this->setExpectedException('EbayEnterprise_Eb2cCore_Exception_Feed_File', 'No sent directory configured');
