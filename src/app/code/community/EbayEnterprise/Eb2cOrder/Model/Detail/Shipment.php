...
class EbayEnterprise_Eb2cOrder_Model_Detail_Shipment
	...
	protected function _construct()
	{
		...
		if (trim($this->getIncrementId())) {
			...
			foreach ($this->getShippedItemIds() as $itemRefId) {
				...
				if ($item && $item->getSku()) {
					$this->_injectShipmentItems(array_merge($item->getData(), array('qty' => $item->getQtyShipped())));
				}
			}
			$tracks = $this->getTracks();
			if (!empty($tracks)) {
				foreach ($tracks as $track) {
					$this->_injectShipmentTracks(array('number' => $track));
				}
			}
