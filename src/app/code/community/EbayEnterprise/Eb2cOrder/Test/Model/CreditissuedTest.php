...
class EbayEnterprise_Eb2cOrder_Test_Model_CreditissuedTest extends EbayEnterprise_Eb2cCore_Test_Base
{
	...
	protected $_payload;
	/** @var  EbayEnterprise_Eb2cOrder_Model_CreditIssued $_creditissued */
	...
	{
		$order = $this->getModelMock('sales/order', array('getIncrementId'), false, array(array('entity_id' => $entityId)));
		$this->replaceByMock('model', 'sales/order', $order);

		$credit = Mage::getModel('eb2corder/creditissued', array('payload' => $this->_payload));
		$returned = EcomDev_Utils_Reflection::invokeRestrictedMethod($credit, '_getOrder', array($orderId));
	...
	public function testCreditmemoInitData($payload, array $itemData, $expectedReturn)
	{
		...
		$order = $this->getModelMock('sales/order', array('getItemsCollection'));
		...
		$order = $this->_injectProtectedProperty('EbayEnterprise_Eb2cOrder_Model_Creditissued', $credit, '_order', $order);
	...
	public function testFixupTotals($subtotal, $grandTotal, $expectedReturn)
	{
		...
		$memo = $this->getModelMock('sales/order_creditmemo', array('getBaseSubtotal'));
		...
		$credit = Mage::getModel('eb2corder/creditissued', array('payload' => $this->_payload));
		$returned = EcomDev_Utils_Reflection::invokeRestrictedMethod($credit, '_fixupTotals', array($memo));
		$actual = array(
