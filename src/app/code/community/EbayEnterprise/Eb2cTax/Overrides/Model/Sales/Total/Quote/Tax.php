...
class EbayEnterprise_Eb2cTax_Overrides_Model_Sales_Total_Quote_Tax extends Mage_Tax_Model_Sales_Total_Quote_Tax
{
	...
	protected function _calcTaxForItem($itemSelector)
	{
		...
		if (Mage::helper('eb2ctax')->getApplyTaxAfterDiscount()) { // tax only what you pay
			...
			$baseRowTax = $this->_calculator->getTax($itemSelector, EbayEnterprise_Eb2cTax_Overrides_Model_Calculation::MERCHANDISE_TYPE);
			// amount to adjust tax due to discount.
			$baseRowTaxDiscount = $this->_calculator->getDiscountTax($itemSelector, EbayEnterprise_Eb2cTax_Overrides_Model_Calculation::MERCHANDISE_TYPE);
	...
	protected function _calcShippingTaxes($address)
	{
		...
		$duty = $this->_calculator->getTax($itemSelector, EbayEnterprise_Eb2cTax_Overrides_Model_Calculation::DUTY_TYPE);
		$baseTax = $this->_calculator->getTax($itemSelector, EbayEnterprise_Eb2cTax_Overrides_Model_Calculation::SHIPPING_TYPE) + $duty;
		...
		if (Mage::helper('eb2ctax')->getApplyTaxAfterDiscount()) {
			$baseTaxDiscount = $this->_calculator->getDiscountTax($itemSelector, EbayEnterprise_Eb2cTax_Overrides_Model_Calculation::SHIPPING_TYPE);
