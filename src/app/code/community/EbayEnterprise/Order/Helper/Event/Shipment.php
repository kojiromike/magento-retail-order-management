...
class EbayEnterprise_Order_Helper_Event_Shipment
{
	...
	protected function _derivePartialTrackData($trackingUrl)
	{
		...
		foreach ($this->_getCarriers() as $code => $title) {
			if (strpos($host, $code) !== false) {
				return ['carrier_code' => $code, 'title' => $title];
			}
		}
		return ['carrier_code' => static::CUSTOM_CARRIER_CODE, 'title' => static::TRACKING_TITLE];
	}
	/**
