...
class EbayEnterprise_Order_Model_Search_Process_Response_Map implements EbayEnterprise_Order_Model_Search_Process_Response_IMap
{
    ...
    {
        return $this->_extractData($summary);
    }
     ...
     * @return array
     */
    protected function _extractData(IOrderSummary $summary)
    {
        $data = [];
        foreach ($this->_config->mapSearchResponse as $key => $callback) {
            if ($callback['type'] !== 'disabled') {
                $getter = $callback['getter'];
                $callback['parameters'] = [$summary, $getter];
