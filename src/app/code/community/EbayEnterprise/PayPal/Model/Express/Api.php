...
class EbayEnterprise_Paypal_Model_Express_Api
{
	...
	{
		$sdk = $this->_getSdk($this->_helper->getConfigModel()->apiOperationSetExpressCheckout);
		...
		if (!$reply->isSuccess() || is_null($reply->getToken())) {
			$this->_logger->logWarn('[%s] SetExpressCheckout request failed: %s', array(__CLASS__, $reply->getErrorMessage()));
			throw Mage::exception('EbayEnterprise_PayPal', $this->_helper->__(self::EBAYENTERPRISE_PAYPAL_API_FAILED));
		}
	...
	{
		$sdk = $this->_getSdk($this->_helper->getConfigModel()->apiOperationGetExpressCheckout);
		$this->_logger->logDebug('[%s] GetExpressCheckout started', array(__CLASS__));
		$payload = $sdk->getRequestBody();
		...
		$this->_logger->logDebug('[%s] GetExpressCheckout completed', array(__CLASS__));
		return array(
...
//				'reply' => $address->getAddressStatus(),
	 ...
	 * @return array
	 */
	protected function _convertToAddressData(Payload\Payment\IPayPalAddress $address)
	{
		return array(
	...
	{
		$sdk = $this->_getSdk($this->_helper->getConfigModel()->apiOperationDoExpressCheckout);
		$payload = $sdk->getRequestBody();
		$payload->setRequestId($this->_coreHelper->generateRequestId(self::PAYPAL_DOEXPRESS_REQUEST_ID_PREFIX))
		...
		if (!$reply->isSuccess()) {
			$this->_logger->logWarn('[%s] PayPal request failed', array(__CLASS__));
			throw Mage::exception('EbayEnterprise_PayPal', $this->_helper->__($reply->getErrorMessage()));
		}
		return array(
			'order_id'       => $reply->getOrderId(),
	...
	{
		$sdk = $this->_getSdk($this->_helper->getConfigModel()->apiOperationDoAuthorization);
		$payload = $sdk->getRequestBody();
		$payload->setRequestId($this->_coreHelper->generateRequestId(self::PAYPAL_DOAUTHORIZATION_REQUEST_ID_PREFIX))
			->setOrderId($quote->reserveOrderId()->getReservedOrderId())
			->setCurrencyCode($quote->getQuoteCurrencyCode())
			->setAmount($this->_getTotal('grand_total', $quote));
		$sdk->setRequestBody($payload);
		$this->_logger->logDebug('[%s] Sending DoAuthorization', array(__CLASS__));

		$this->_sendRequest($sdk);
		$reply = $sdk->getResponseBody();
		$this->_logger->logDebug('[%s] Received DoAuthorization response', array(__CLASS__));
	...
	{
		$sdk = $this->_getSdk($this->_helper->getConfigModel()->apiOperationDoVoid);
		$payload = $sdk->getRequestBody();
		...
		$this->_sendRequest($sdk);
		$reply = $sdk->getResponseBody();
		$this->_logger->logDebug('[%s] Received DoVoid response', array(__CLASS__));
		$isVoided = $reply->isSuccess();
		if (!$reply->isSuccess()) {
			$this->_logger->logWarn('[%s] PayPal DoVoid failed', array(__CLASS__));
			throw Mage::exception('EbayEnterprise_PayPal', $this->_helper->__(static::EBAYENTERPRISE_PAYPAL_API_FAILED));
		}
		return array(
			'order_id'  => $reply->getOrderId(),
			'is_voided' => $isVoided
	 ...
	 * @return self
	 */
	protected function _addShippingAddress(Mage_Sales_Model_Quote_Address $shippingAddress, Payload\Payment\IShippingAddress $payload)
	{
	...
	{
		try {
			$this->_logger->logDebug("[%s] Sending request:\n%s", array(__CLASS__, $sdk->getRequestBody()->serialize()));
			$sdk->send();
			$reply = $sdk->getResponseBody();
			$this->_logger->logDebug("[%s] Received reply:\n%s", array(__CLASS__, $reply->serialize()));
		} catch (Payload\Exception\InvalidPayload $e) {
			$this->_logger->logWarn("[%s] PayPal payload invalid:\n%s", array(__CLASS__, $e));
			throw Mage::exception('EbayEnterprise_PayPal', $this->_helper->__(static::EBAYENTERPRISE_PAYPAL_API_FAILED));
		} catch (Api\Exception\NetworkError $e) {
			$this->_logger->logWarn("[%s] PayPal request failed:\n%s", array(__CLASS__, $e));
			throw Mage::exception('EbayEnterprise_PayPal', $this->_helper->__(static::EBAYENTERPRISE_PAYPAL_API_FAILED));
		}
		return $this;
	}
	 ...
	 */
	protected function _addLineItems(Mage_Sales_Model_Quote $quote, Payload\Payment\ILineItemContainer $container)
	{
		...
		foreach (array('discount', 'giftcardaccount', 'ebayenterprise_giftcard') as $totalType) {
	 ...
	 * @return self
	 */
	protected function _processItem(Mage_Sales_Model_Quote_Item_Abstract $item, Payload\Payment\ILineItemIterable $lineItems, $currencyCode)
	{
