...
class EbayEnterprise_Paypal_Model_Express_Checkout
{
	...
	{
		list($this->_helper, $this->_logger, $this->_config, $this->_quote) = $this->_checkTypes(
	...
	public function start($returnUrl, $cancelUrl, $button = null)
	{
		...
		if (!$this->_quote->getGrandTotal() && !$this->_quote->hasNominalItems()) {
			Mage::throwException(Mage::helper('ebayenterprise_paypal')->__(self::EBAYENTERPRISE_PAYPAL_ZERO_CHECKOUT_NOT_SUPPORTED));
		}
	...
	*/
	...
	public function returnFromPaypal($token)
	{
		$this->_getApi();
		$quote = $this->_quote;
		$getExpressCheckoutReply = $this->_api->getExpressCheckout($quote->reserveOrderId()->getReservedOrderId(), $token,
		...
		$payment
			->setAdditionalInformation(self::PAYMENT_INFO_TRANSPORT_PAYER_ID, $getExpressCheckoutReply['payer_id'])
	...
	public function place($token, $shippingMethodCode = null)
	{
		...
		$this->_quote->collectTotals();
		$this->_getApi();
		$payerId = $this->_quote->getPayment()->getAdditionalInformation(self::PAYMENT_INFO_TRANSPORT_PAYER_ID);
		...
		$this->_quote->getPayment()->importData(array_merge(
			$doExpressReply,
	...
	{
		$shipping = $this->quote->isVirtual() ? null : $this->_quote->getShippingAddress();
