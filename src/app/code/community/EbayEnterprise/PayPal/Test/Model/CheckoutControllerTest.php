...
class EbayEnterprise_PayPal_Test_Model_CheckoutControllerTest
	extends EbayEnterprise_Eb2cCore_Test_Base
{
	...
	{
		$this->_api = $this->getModelMock(
			'ebayenterprise_paypal/api_express', array('setExpressCheckout')
		);
		$this->_helper = $this->getHelperMock('ebayenterprise_paypal/data');
		$this->_helper->expects($this->any())
			->method('__')->with($this->isType('string'))
			->will($this->returnArgument(0));
		$this->_expressMethod = $this->getModelMock(
			'ebayenterprise_paypal/method_express',
			array('start', 'getStartRedirectUrl', 'assignData')
		);
		$payment = $this->getModelMock(
			'sales/quote_payment', array('getMethodInstance')
		);
		$payment->expects($this->any())
			->method('getMethodInstance')->will(
				$this->returnValue($this->_expressMethod)
			);
		...
		$this->_session = $this->getModelMockBuilder('core/session')
			->disableOriginalConstructor()
			->setMethods(array('addError'))
			->getMock();
		$this->_checkoutHelper = $this->getHelperMock(
			'checkout/data', array('getCheckout', 'getQuote')
		);
	...
	{
		$this->replaceByMock(
			'model', 'ebayenterprise_paypal/api_express', $this->_api
		);
		$this->_api->expects($this->once())
			->method('setExpressCheckout')->will(
				$this->throwException(Mage::exception('EbayEnterprise_PayPal'))
			);
		$this->_session->expects($this->any())
			->method('addError')->with($this->isType('string'));
		$controller = $this->getMockBuilder(
			'EbayEnterprise_PayPal_CheckoutController'
		)->disableOriginalConstructor()
			->setMethods(array('_redirect'))
			->getMock();
		print_r(get_class($controller));
		$controller->expects($this->once())
			->method('_redirect')->with($this->identicalTo('checkout/cart'))
	...
	public function provideForStartAction()
	{
		return array(
			array(true,
			      'https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=thetokenstring'),
			array(false,
			      'https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=thetokenstring'),
		);
	}
	...
	public function testStartAction($isSandboxedFlag, $expectedUrl)
	{
		...
		$config = $this->buildCoreConfigRegistry(
			array('isSandboxedFlag' => $isSandboxedFlag)
		);
		$this->replaceByMock(
			'model', 'ebayenterprise_paypal/api_express', $this->_api
		);
		$this->_api->expects($this->once())
			->method('setExpressCheckout')->with(
				$this->isInstanceOf('Mage_Sales_Model_Quote')
			)
		...
		$controller = $this->getMockBuilder(
			'EbayEnterprise_PayPal_ExpressController'
		)->disableOriginalConstructor()
		...
		EcomDev_Utils_Reflection::setRestrictedPropertyValues(
			$controller, array(
				'_checkoutHelper' => $this->_checkoutHelper,
				'_config'         => $config
			)
		);
		$controller->startAction();
	...
	public function testReturnAction($isVirtualQuote = false)
	{
		...
		$this->_api->expects($this->once())
			->method('getExpressCheckout')->with(
