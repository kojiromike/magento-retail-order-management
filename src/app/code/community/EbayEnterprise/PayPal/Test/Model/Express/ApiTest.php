...
class EbayEnterprise_PayPal_Test_Model_Express_ApiTest extends EbayEnterprise_Eb2cCore_Test_Base
{
	...
	public function setUp()
	{
		...
		$this->_coreUrl = $this->getModelMock('core/url', array('_construct', 'getUrl'));
		$this->_coreUrl->expects($this->any())
			->method('getUrl')->will($this->returnValueMap(array(
		...
		$this->_coreHelper = $this->getHelperMock('eb2ccore/data', array('getSdkApi'));
		$this->_coreHelper->expects($this->any())
			->method('getSdkApi')->will($this->returnValueMap($this->_getSdkApiMap));
		$this->_helper = $this->getHelperMock('ebayenterprise_paypal/data', array('getConfigModel', '__'));
		$this->_helper->expects($this->any())->method('__')->will($this->returnArgument(0));
		$this->_quote = $this->getModelMock('sales/quote', array('reserveOrderId', 'getReservedOrderId', 'getAllNonNominalItems', 'getTotals'));
		$this->_quote->expects($this->any())
			->method('reserveOrderId')->will($this->returnSelf());
		$this->_quote->expects($this->any())
			->method('getTotals')->will($this->returnValue(array(
				'grand_total' => new Varien_Object(array('value'=>100)),
				'shipping' => new Varien_Object(array('value'=>5.95)),
				'tax' => new Varien_Object(array('value'=>2.50)),
			)));
		$this->_quote->expects($this->any())
			->method('getReservedOrderId')->will($this->returnValue('orderid'));
		$this->_quote->expects($this->any())
			->method('getAllNonNominalItems')->will($this->returnValue(array()));
	...
	public function testSetExpressCheckoutWithSdkException($exception)
	{
		...
		$this->_sdk->expects($this->exactly(2))
			->method('getRequestBody')->will($this->returnValue($setExpressRequest));
		$this->_sdk->expects($this->once())
			->method('setRequestBody')->with($this->isInstanceOf(self::SETEXPRESS_REQUEST_PAYLOAD))
		...
		$api = $this->getModelMock('ebayenterprise_paypal/express_api', array('_addLineItems', '_addShippingAddress'));
		EcomDev_Utils_Reflection::setRestrictedPropertyValues($api, array(
	...
	public function testStartWithFailureResponse()
	{
		...
		// test setExpressCheckout
		$message = EbayEnterprise_PayPal_Model_Method_Express::PAYPAL_CHECKOUT_FAILED_MESSAGE;
		$api = $this->getModelMock('ebayenterprise_paypal/express_api', array('_prepareRequest'));
		EcomDev_Utils_Reflection::setRestrictedPropertyValue($api, '_helper', $this->_helper);
		$this->setExpectedException('EbayEnterprise_PayPal_Exception', $message);
	...
	public function testSetExpressCheckout()
	{
		...
		$setExpressRequest->expects($this->once())
			->method('setOrderId')->with($this->identicalTo('orderid'))->will($this->returnSelf());
		$setExpressRequest->expects($this->once())
			->method('setReturnUrl')->with($this->identicalTo('the.return.url'))->will($this->returnSelf());
		$setExpressRequest->expects($this->once())
			->method('setCancelUrl')->with($this->identicalTo('the.cancel.url'))->will($this->returnSelf());
		$setExpressRequest->expects($this->once())
			->method('setLocaleCode')->with($this->identicalTo('en_US'))->will($this->returnSelf());
		$setExpressRequest->expects($this->once())
			->method('setAmount')->with($this->identicalTo(100))->will($this->returnSelf());
		$setExpressRequest->expects($this->once())
			->method('setCurrencyCode')->with($this->identicalTo('USD'))->will($this->returnSelf());
		...
		$this->_sdk->expects($this->once())
			->method('send')->will($this->returnSelf());
		$this->_sdk->expects($this->once())
			->method('getResponseBody')->will($this->returnValue($setExpressReply));
		$setExpressReply->expects($this->any())
			->method('isSuccess')->will($this->returnValue(true));

		// test setExpressCheckout
		$api = $this->getModelMock('ebayenterprise_paypal/express_api', array('_addShippingAddress'));
		EcomDev_Utils_Reflection::setRestrictedPropertyValues($api, array(
			'_helper' => $this->_helper, '_coreHelper' => $this->_coreHelper
		));
		$result = $api->setExpressCheckout($this->_quote);
		$this->assertEquals(array('token'=>'theToken', 'order_id' => 'orderid'), $result);
	}
	...
	public function testGetExpressCheckout()
	{
		...
		$getExpressRequest->expects($this->once())
			->method('setOrderId')->with($this->identicalTo('orderid'))->will($this->returnSelf());
		$getExpressRequest->expects($this->once())
			->method('setToken')->with($this->identicalTo('thetoken'))->will($this->returnSelf());
		$getExpressRequest->expects($this->once())
			->method('setCurrencyCode')->with($this->identicalTo('USD'))->will($this->returnSelf());
		...
		$this->_sdk->expects($this->atLeastOnce())
			->method('getRequestBody')->will($this->returnValue($getExpressRequest));
		// setup the sdk and populate a request payload.
		$this->_sdk->expects($this->once())
			->method('setRequestBody')->with($this->isInstanceOf(self::GETEXPRESS_REQUEST_PAYLOAD))
			->will($this->returnSelf());
		$this->_sdk->expects($this->once())
			->method('send')->will($this->returnSelf());
		$this->_sdk->expects($this->atLeastOnce())
			->method('getResponseBody')->will($this->returnValue($getExpressReply));
