...
class EbayEnterprise_PayPal_Test_Model_Method_ExpressTest extends EbayEnterprise_Eb2cCore_Test_Base
{
	...
	public function setUp(){
		$this->_paymentInfo = $this->getModelMock('ebayenterprise_paypal/express_payment_info', array('setAdditionalInformation'));
	}
	/**
	 * verify:
	...
	public function testIsRegisteredAsPaymentMethod()
	{
		...
		$method = Mage::helper('payment')->getMethodInstance('ebayenterprise_paypal_express');
	...
	public function testAssignData($isDataObject)
	{
		...
		$this->_paymentInfo->expects($this->once())
			->method('setAdditionalInformation')->with($this->identicalTo('token'), $this->identicalTo('thetokenstring'))->will($this->returnSelf());
