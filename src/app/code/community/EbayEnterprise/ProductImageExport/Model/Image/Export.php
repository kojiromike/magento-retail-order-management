...
class EbayEnterprise_ProductImageExport_Model_Image_Export extends Varien_Object
{
	...
	{
		foreach (array_keys(Mage::helper('ebayenterprise_catalog')->getStores()) as $storeId) {
	...
	{
		$pHelper = Mage::helper('ebayenterprise_catalog');
	...
	{
		$domainParts = parse_url(Mage::helper('ebayenterprise_catalog')->getStoreUrl($storeId));
	...
	protected function _generateFilePath($storeId)
	{
		...
		$coreFeed = Mage::getModel('ebayenterprise_catalog/feed_core', array('feed_config' => $cfg->imageFeed));
	...
	protected function _validateXml(EbayEnterprise_Dom_Document $doc)
	{
		Mage::getModel('eb2ccore/api')->schemaValidate(
			$doc, Mage::helper('ebayenterprise_catalog')->getConfigModel()->imageExportXsd
		);
