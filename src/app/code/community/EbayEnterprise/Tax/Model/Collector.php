...
class EbayEnterprise_Tax_Model_Collector
{
	...
	public function getTaxRecordsByAddressId($addressId)
	{
		return array_filter(
			$this->getTaxRecords(),
			function ($record) use ($addressId) { return $record->getAddressId() === $addressId; }
		);
	}
	...
	public function getTaxRecordsByItemId($itemId)
	{
		return array_filter(
			$this->getTaxRecords(),
			function ($record) use ($itemId) { return $record->getItemId() === $itemId; }
		);
	}
	...
	public function getTaxDutiesByAddressId($addressId)
	{
		return array_filter(
			$this->getTaxDuties(),
			function ($duty) use ($addressId) { return $duty->getAddressId() === $addressId; }
		);
	}
	...
	public function getTaxFeesByItemId($itemId)
	{
		return array_filter(
			$this->getTaxFees(),
			function ($fee) use ($itemId) { return $fee->getItemId() === $itemId; }
		);
	}
	...
	public function getTaxFeesByAddressId($addressId)
	{
		return array_filter(
			$this->getTaxFees(),
			function ($fee) use ($addressId) { return $fee->getAddressId() === $addressId; }
		);
	}
