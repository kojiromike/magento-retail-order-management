...
class EbayEnterprise_Tax_Model_Total_Quote_Address_Tax
	extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
	...
	protected function _totalTaxRecordsCalculatedTaxes(array $taxRecords)
	{
		return array_reduce(
			$taxRecords,
			function ($total, $taxRecord) { return $total + $taxRecord->getCalculatedTax(); },
			0.00
		);
	}
	...
	protected function _totalDuties(array $duties)
	{
		return array_reduce(
			$duties,
			function ($total, $duty) { return $total + $duty->getAmount(); },
			0.00
		);
	}
	...
	protected function _totalFees(array $fees)
	{
		return array_reduce(
			$fees,
			function ($total, $fee) { return $total + $fee->getAmount(); },
			0.00
		);
	}
}
