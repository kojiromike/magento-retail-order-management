...
class TrueAction_Eb2cTax_Test_Model_RequestTest extends TrueAction_Eb2cCore_Test_Base
{
	...
	public function testVirtualPhysicalMix()
	{
		...
		$x->registerNamespace('a', $doc->documentElement->namespaceURI);
		$this->assertSame(
			$x->evaluate('string(./a:PersonName/a:LastName)',
			$nodeA),
			$x->evaluate('string(./a:PersonName/a:LastName)',
			$nodeB)
		);
		$this->assertSame(
			$x->evaluate('string(./a:PersonName/a:FirstName)',
			$nodeA),
			$x->evaluate('string(./a:PersonName/a:FirstName)',
			$nodeB)
		);
		$this->assertSame(
			$x->evaluate('string(./a:Address/a:Line1)',
			$nodeA),
			$x->evaluate('string(./a:Address/a:Line1)',
			$nodeB)
		);
		$this->assertSame(
			$x->evaluate('string(./a:Address/a:City)',
			$nodeA),
			$x->evaluate('string(./a:Address/a:City)',
			$nodeB)
		);
		$this->assertSame(
			$x->evaluate('string(./a:Address/a:MainDivision)',
			$nodeA),
			$x->evaluate('string(./a:Address/a:MainDivision)',
			$nodeB)
		);
		$this->assertSame(
			$x->evaluate('string(./a:Address/a:CountryCode)',
			$nodeA),
			$x->evaluate('string(./a:Address/a:CountryCode)',
			$nodeB)
		);
		$this->assertSame(
			$x->evaluate('string(./a:Address/a:PostalCode)',
			$nodeA),
	...
	public function testCheckDiscountsEmptyQuote()
	{
		...
		$address->setData(array('address_id' => 1,
			'quote_id'                    => 1,
			'customer_id'                 => 5,
			'save_in_address_book'        => 1,
			'customer_address_id'         => 4,
